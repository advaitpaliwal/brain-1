...
class TrimodalFeatureDataset(Dataset[dict[str, Any]]):
    def __init__(self, manifest_path: str | Path) -> None:
        self.manifest_path = Path(manifest_path)
        self.records = self._load_records()

    def _load_records(self) -> list[TrimodalManifestRecord]:
        rows = [json.loads(line) for line in self.manifest_path.read_text(encoding="utf-8").splitlines()]
        return [
            TrimodalManifestRecord(
                dataset_name=row["dataset_name"],
                subject_id=row["subject_id"],
                subject_index=int(row["subject_index"]),
                stimulus_id=row["stimulus_id"],
                text_feature_path=Path(row["text_feature_path"]),
                audio_feature_path=Path(row["audio_feature_path"]),
    ...
    def __getitem__(self, index: int) -> dict[str, Any]:
        ...
        audio = torch.load(record.audio_feature_path, map_location="cpu")["features"].float()
        ...
        length = min(text.shape[0], audio.shape[0], video.shape[0], target.shape[0])
        return {
...
def collate_trimodal_batch(batch: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    audio = torch.zeros(len(batch), max_len, audio_dim, dtype=torch.float32)
    ...
    for row_index, item in enumerate(batch):
        ...
        audio[row_index, :length] = item["audio_features"]
